...
def generate_description_from_cdp(filename):
    regex = (r'(\w+)\s+(\S+ \d+/\d+)\s+.*?(\S+ \d+/\d+)')
    result_dict = {}
    with open(filename) as f:
        for value in re.finditer(regex, f.read(), re.DOTALL):
            result_dict[value.group(2)] = 'description Connected to {0} port {1}'.format(value.group(1), value.group(3))
    return result_dict
