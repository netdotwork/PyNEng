...
#       print(uniq_repeated_email_list, end = '\n\n')

    with open(source_file, encoding = 'UTF-8') as f, open(output_file, 'w', encoding = 'UTF-8') as ff:
        reader = csv.reader(f)
...
#               row[2] = convert_datetimestr_to_datetime(row[2])
                writer.writerow(row)
#       print(sorted(new_list, key=itemgetter(1)))
